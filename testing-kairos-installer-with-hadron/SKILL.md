...
description: Use when you need to test a kairos-installer change end-to-end on a real Kairos/Hadron ISO — building the image, overlaying kairos-agent from master, booting in QEMU, driving the interactive installer headlessly, triggering install failures, and recording a video of the run.
---
...
This is a battle-tested runbook (validated end-to-end). Helper scripts live
beside this file: `qmp.py`, `drive.py`, `record-loop.sh`, `mkvideo.sh`,
`fake-agent`, `Dockerfile.installer`.
...
slot. See `Dockerfile.installer`:
```bash
cp /tmp/build/kairos-installer /tmp/build/kairos-agent-main fake-agent /tmp/build/
cd /tmp/build && docker build -q -t hadron-init-installer:test -f /path/to/Dockerfile.installer .
```
...
### 7. Drive the installer headlessly (see `drive.py`)
```bash
D="python3 drive.py /tmp/ke.sock"   # NOTE: zsh won't word-split this; call python3 directly
python3 drive.py /tmp/ke.sock type 'kairos-agent interactive-install'   # dispatch path
python3 drive.py /tmp/ke.sock key ret
python3 drive.py /tmp/ke.sock shot /tmp/build/frames/01.ppm             # then convert to png
python3 drive.py /tmp/ke.sock key ctrl-d                                # debug bundle hotkey
python3 drive.py /tmp/ke.sock key 'down down ret'                       # menus
```
...
```bash
./record-loop.sh /tmp/ke.sock /tmp/build/rec 1 &     # screendump every 1s
# ... drive the installer ...
touch /tmp/build/rec/.stop
ffmpeg -y -framerate 6 -i /tmp/build/rec/%06d.png -c:v libx264 -pix_fmt yuv420p full.mp4
```
For a curated clip from specific screenshots, give `mkvideo.sh` (png,seconds)
pairs: `./mkvideo.sh out 25  disk.png 3  ready.png 4 ...` → `out.mp4` + `out.gif`.
...
### 9. Trigger an install failure (see `fake-agent`)
...
delegates `logs` to the real agent (so the bundle still gathers journald):
```bash
# fake-agent is baked into the image (see Dockerfile.installer) at /opt/fake-agent
python3 drive.py /tmp/ke.sock type 'export KAIROS_AGENT_BIN=/opt/fake-agent ; kairos-agent interactive-install'
python3 drive.py /tmp/ke.sock key ret
...
```bash
python3 qmp.py /tmp/ke.sock "hostfwd_add tcp::35791-:<GUEST_PORT>"   # use a FREE host port
...
| Symptom | Cause / Fix |
|---|---|
| Video shows wrong screen / one frame for the whole clip | ffmpeg concat-demuxer image `duration` is unreliable. Use `mkvideo.sh` (numbered sequence). **Always extract frames from the finished mp4 and eyeball them.** |
...
| `drive.py type` errors `unmapped char` | Add the char to `CHARMAP` in `drive.py`. Avoid host-side `$(...)`/globs leaking into the typed string — single-quote the argument. |
...
- Helper scripts: `qmp.py` (one HMP cmd), `drive.py` (type/key/shot), `record-loop.sh` (boot-to-end capture), `mkvideo.sh` (curated clip), `fake-agent` (failure shim), `Dockerfile.installer` (derivative image).
